...
{
	public class TestAIPilots : ITestCase
	{
		...
		public void SetupTest()
		{
			...
			if (TestAIPilots.<>f__mg$cache0 == null)
			{
				TestAIPilots.<>f__mg$cache0 = new UnityAction(TestAIPilots.OnFinishedLoading);
			}
			onFinishLoading.AddListener(TestAIPilots.<>f__mg$cache0);
		...
		private static void OnFinishedLoading()
		{
			...
			if (TestAIPilots.<>f__mg$cache1 == null)
			{
				TestAIPilots.<>f__mg$cache1 = new UnityAction<Tuple<string, string>>(TestAIPilots.OnCollision);
			}
			collisionEvent.AddListener(TestAIPilots.<>f__mg$cache1);
			CrewSim.objInstance.NewGame("OKLG_AND_NEARBYSTATION");
			CrewSim.objInstance.DebugSetPlayerStartingPosition("OKLG", 60f, null);
			UnityEvent onFinishLoading = CrewSim.OnFinishLoading;
			if (TestAIPilots.<>f__mg$cache2 == null)
			{
				TestAIPilots.<>f__mg$cache2 = new UnityAction(TestAIPilots.OnFinishedLoading);
			}
			onFinishLoading.RemoveListener(TestAIPilots.<>f__mg$cache2);
		}
		...
		private IEnumerator HaulerDeployingPilotTest()
		{
			...
			TestManager.AssertIsTrue(derelict.objSS.bBOLocked, "Derelict was anchored");
			yield return new WaitUntil(() => ai.Ship.HideFromSystem && !ai.Ship.bDestroyed);
			TestManager.Log("Hauler DockedAndDespawned", LogColor.Green);
			yield break;
		}

		[TestCase(Title = "HaulerDeployerCrash", Description = "Spawns Hauler with derelict and crashes it into the BO", Order = 4)]
		private IEnumerator HaulerDeployingCrashTest()
		{
			...
			AIShip ai = AIShipManager.SpawnAI(AIType.HaulerDeployer, this._middlePos.vPosx, this._middlePos.vPosy + 1E-09, false);
			TestManager.AssertIsTrue(ai != null && ai.Ship != null, "Ship was spawned");
			BodyOrbit bo = CrewSim.system.GetNearestBO(ai.Ship.objSS, StarSystem.fEpoch, false);
			TestManager.Log("Hauler Created! Steering it into BO", LogColor.Green);
			while (ai.Ship != null && !ai.Ship.bDestroyed)
			{
				AIShipManager.AIIntercept2(ai.Ship, bo.dXReal, bo.dYReal, bo.dVelX, bo.dVelY, 0.0, null, 0.0, 0.0);
				yield return null;
			}
		...
		public void Teardown()
		{
			...
			{
				UnityEvent<Tuple<string, string>> collisionEvent = CollisionManager.CollisionEvent;
				if (TestAIPilots.<>f__mg$cache3 == null)
				{
					TestAIPilots.<>f__mg$cache3 = new UnityAction<Tuple<string, string>>(TestAIPilots.OnCollision);
				}
				collisionEvent.RemoveListener(TestAIPilots.<>f__mg$cache3);
			}
		}

	...
	public class JsonMapper
	{
		...
		private static object ReadValue(Type inst_type, JsonReader reader)
		{
			...
			else
			{
				if (reader.Token != JsonToken.Double && reader.Token != JsonToken.Int && reader.Token != JsonToken.Single && reader.Token != JsonToken.Long && reader.Token != JsonToken.String && reader.Token != JsonToken.Boolean)
				{
					...
					else if (reader.Token == JsonToken.ObjectStart)
					{
						...
						for (;;)
						{
							...
							if (objectMetadata.Properties.TryGetValue(text, out propertyMetadata))
							{
				...
				if (JsonMapper.custom_importers_table.TryGetValue(type2, out dictionary) && dictionary.TryGetValue(type, out importerFunc))
				{
					return importerFunc(reader.Value);
				}
				if (JsonMapper.base_importers_table.TryGetValue(type2, out dictionary) && dictionary.TryGetValue(type, out importerFunc))
				{
					return importerFunc(reader.Value);
				}
				if (type.IsEnum)
		...
		private static void RegisterBaseImporters()
		{
			...
			importer = ((object input) => Convert.ToSingle((decimal)input));
			JsonMapper.RegisterImporter(JsonMapper.base_importers_table, typeof(float), typeof(decimal), importer);
		...
		private static void WriteValue(object obj, JsonWriter writer, bool writer_is_private, int depth)
		{
			...
			foreach (PropertyMetadata propertyMetadata in list)
			{
						...
						if (!(value is Array) || ((Array)value).Length != 0)
						{
							if (!(value is IList) || ((IList)value).Count != 0)
							{
								if (!(value is IDictionary) || ((IDictionary)value).Count != 0)
								{
									...
									JsonMapper.WriteValue(value, writer, writer_is_private, depth + 1);
								}
							}
						}
					}
				...
				else
				{
					...
					if (value2 != null)
					{
						if (!(value2 is Array) || ((Array)value2).Length != 0)
						{
							if (!(value2 is IList) || ((IList)value2).Count != 0)
							{
								if (!(value2 is IDictionary) || ((IDictionary)value2).Count != 0)
								{
									if (propertyInfo.CanRead)
									{
										writer.WritePropertyName(propertyMetadata.Info.Name);
										JsonMapper.WriteValue(value2, writer, writer_is_private, depth + 1);
									}
								}
							}
						}

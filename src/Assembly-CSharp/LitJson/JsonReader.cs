...
namespace LitJson
{
	public class JsonReader
	{
		...
		private void ProcessNumber(string number)
		{
			...
			{
				if (number.IndexOf("9e99", StringComparison.Ordinal) >= 0)
				{
					this.token = JsonToken.Single;
					float num;
					if (number[0] == '-')
					{
			...
			int num3;
			if (int.TryParse(number, out num3))
			{
			...
			long num4;
			if (long.TryParse(number, out num4))
			{
			...
			ulong num5;
			if (ulong.TryParse(number, out num5))
			{
				this.token = JsonToken.Long;

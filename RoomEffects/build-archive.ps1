...
$modInfoPath = ".\mod_data\Room_Effects\mod_info.json"
...
# Build the project
Write-Host "Building RoomEffects in Release configuration..."
...
$possibleDllPaths = @(
    ".\bin\Release\RoomEffects.dll",
    ".\bin\Release\net472\RoomEffects.dll",
    ".\bin\Release\net48\RoomEffects.dll",
    ".\bin\Release\net6.0\RoomEffects.dll"
)
...
foreach ($dllPath in $possibleDllPaths) {
    if (Test-Path $dllPath) {
        ...
        Copy-Item -Path $dllPath -Destination ".\release_archive\BepInEx\plugins\RoomEffects.dll" -Force
        $dllFound = $true
        break
    }
}
...
Write-Host "Creating archive RoomEffects-v$currentVersion.zip..."
$zipPath = ".\RoomEffects-v$currentVersion.zip"
...
if (Test-Path $zipPath) {
    ...
    Write-Host "Archive saved as: RoomEffects-v$currentVersion.zip ($('{0:F2}' -f $zipSize) MB)"
}
else {

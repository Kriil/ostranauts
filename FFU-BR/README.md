...
**JsonLogging** - defines if JSON parsing errors will be shown in the log during loading. Applies
only, when there are actual JSON-related errors (i.e. reading unstable dynamic object and failing).  
...
values: Top, Bottom, Horizontal, Vertical. **Top Offset** - vertical offset, which defines initial inventory 
window position. **Bottom Limit** - horizontal cutout offset, below which inventory window will be shown in 
next column, required to balance the 'top' offset. **Horizontal Padding** - padding between columns of 
inventory windows. **Vertical Padding** - padding between each window in the column.  
...
**SuperCharacters** - List of exact character names that you want to apply multiplier to.  
...
**strMathCond** - `condtrigs` parameter that is used to perform various logical operations between value of
`strMathCond` condition and multiple values (or their fractions) of other conditions. In addition, requires 
defined `aMathOps` parameter to work, otherwise it will be ignored. Behavior of the parameter greatly depends
on **bAND** value. If `bAND` is set to `true`, the `strMathCond` will only trigger, if **ALL** conditions listed
in the `aMathOps` are met. If `bAND` is set to `false`, the `strMathCond` will trigger as long as at least 
**ONE** of the conditions listed in the `aMathOps` was met.

**aMathOps** - `condtrigs` parameter that defines operations with conditions that `strMathCond` will perform.
Each valid entry requires 4 different values in object: `{"strID": "OBJ_NAME", "strCond": "COND_NAME",
"nMathOp": "MATH_OP", "fMathValue": "MATH_MULT"}`. As long as `aMathOps` objects are defined correctly, amount of 
possible entries has no limit - only potential performance impact. For example, `"aMathOps": [{"strID": 
"Below20Inc", "strCond": "StatDamageMax", "nMathOp": 3, "fMathValue": 0.2}, {"strID": "Above70Inc", 
"strCond": "StatDamageMax", "nMathOp": 5, "fMathValue": 0.7}]` will return true, if value of the `strMathCond` 
condition is above 20% and below 70% value of the `StatDamageMax` condition. Using `null` instead of `"COND_NAME"` 
in the `aMathOps` will compare with `fMathValue` as value and not as multiplier, thus allowing static value 
operations as well (i.e. `"aMathOps": [{"strID": "AboveValInc", "strCond": null, "nMathOp": 3, "fMathValue": 
12.5}, {"strID": "BelowValInc", "strCond": null, "nMathOp": 5, "fMathValue": 27.5}]`). Objects without
properly defined `strID` (i.e. `null` or empty string) are ignored.

**strCond** - a `aMathOps` object parameter that is used to compare value of a specific condition with 
a specific number. In addition, requires defined `nMathOp` and `fMathVal` parameters to work. Supports number 
of different operations.

**nMathOp** - a `aMathOps` object parameter that defines the math operation that will be used when comparing
condition's value (from `strCond`) with a `fMathVal` parameter value. **1** → Not Equal (`!=`), **2** → 
Equal (`==`), **3** → Greater Than (`>`), **4** → Greater or Equal (`>=`), **5** → Less Than (`<`), **6** → 
Less or Equal (`<=`). **Formula**: *CT.Value #OP# fMathVal*. Invalid value will fail in CT check.

**fMathVal** - a `aMathOps` object parameter that defines the value, which will be used for mathematical 
comparison. If `fMathVal` isn't set, default value of `0` will be used. Whilst you can put negative value in 
`fMathVal` parameter, it is irrelevant, since condition values can't be below zero.
...
*\* Precision Object Modifications* - set of commands to precisely manipulate object arrays in same safe manner.  
...
## Precision Object Modifications
In rare cases, Ostranauts data is stored in array of objects. Unlike dictionaries, which have proper unique key
that easily allows to identify required data, object modification relies on combination of `strID` and `strName`
parameters to lookup for relevant data. Look up via `strID` is given priority and if it doesn't exist in data
block, the `strName` parameter is used for data lookup.

Similar to dictionaries, there are 3 methods to modify the objects:  
`{"strID":"SomeID", //...DATA...//}` adds new or modifies existing object entry with the new data.  
`{"strID":"*SomeID", //...DATA...//}"` removes old object in the record and replaces it with a new one.  
`{"strID":"~SomeID", //...DATA...//}"` completely removes old object along with all the data in it.  

**Note**: if object type has no `strID`, you must use `strName` instead. In addition, this feature works only
on objects that have `strID` or `strName` parameter. If they don't - objects are treated as simple data and
get completely overwritten.
```json
{
    "strName": "OKLG Mescaform",
    "aItems": [
        {
            "strID": "76fcf31c-0191-4a18-ace6-bfd8282b911d",
            "strName": "ItmFloorRYOF01",
            "fX": 5.0,
            "fY": -15.0,
            "fRotation": 180.0
        },
        {
            "strID": "272fdab5-3ace-4e9e-844b-7f752991d4a9",
            "fX": 7.0,
            "fY": -12.0
        },
        {
            "strID": "*76fcf31c-0191-4a18-ace6-bfd8282b911c",
            "strName": "ItmFloorRYOF01",
            "fX": 5.0,
            "fY": -15.0,
            "fRotation": 180.0
        ...
        {
            "strID": "~e6c51f7f-b030-4cbe-9527-d43436029d41"
        }
    ]
}
```
The best example of usage for this modding API is `ships` data. In example above, object array `aItems` gets new
object with id `76fcf31c-0191-4a18-ace6-bfd8282b911d`, overrides existing `272fdab5-3ace-4e9e-844b-7f752991d4a9`
object, completely replaces object `76fcf31c-0191-4a18-ace6-bfd8282b911c` without keeping any previous data and
completely removes object with id `e6c51f7f-b030-4cbe-9527-d43436029d41`. Thus, in similar way to dictionary
modification, object modification allows you to operate at various sub-levels without an issue.
...
and you're pretty much asking for a trouble. Same applies to sub-arrays, except first goes number of modified row
in the array - without it parser will ignore the row, as it won't even be seen as proper sub-array.
...
`StatDismantleProgressMax=1.0x180` was added as is to the list and the parameter `IsGood=1.0x60` was

...
public partial class patch_GUIInventoryItem : GUIInventoryItem {
    ...
    public void OnShiftPointerDown() {
        ...
        if (windowData != null && windowData.type == InventoryWindowType.Container 
            ...
            if (CO.objCOParent.DropCO(CO, false) != null) AttachToCursor();
            return;
        }
        if (windowData == null && CO.objCOParent != null) {
            if (CO.objCOParent.DropCO(CO, false) != null) AttachToCursor();
            return;
        }
        CO.RemoveFromCurrentHome();
        GUIInventory.RemoveTooltip();
        if (windowData != null) windowData.RemoveAndDestroy(CO.strID);
        else {
        ...
        if (addedCO == null) {
            ...
            if (GUIInventory.CTOpenInv.Triggered(CO))
                GUIInventory.instance.SpawnInventoryWindow(CO, InventoryWindowType.Container, null);

...
protected override void OnUpdateUI()
{
	...
	if (_co.HasCond("IsPowerObservable"))
	{
		...
		{
			if (_ctPowerStorage == null)
			{
				_ctPowerStorage = DataHandler.GetCondTrigger("TIsPowerStorage");
			}
			foreach (CondOwner item2 in _co.GetCOsSafe(bAllowLocked: true, _ctPowerStorage))
			{
				if (!(item2 == null))
				{
					num2 += item2.GetCondAmount("StatPowerMax") * item2.GetDamageState();
					num += item2.GetCondAmount("StatPower");
				}
			}
		}
		if (num2 != 0.0)

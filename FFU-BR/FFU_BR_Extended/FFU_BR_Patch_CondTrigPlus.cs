...
public partial class patch_CondTrigger : CondTrigger {
    ...
    public CondTrigger CloneDeep(string strFind, string strReplace) {
        ...
        if (aMathOps != null) {
            condTrigger.aMathOps = new JsonMathOp[aMathOps.Length];
            for (int i = 0; i < aMathOps.Length; i++) {
                condTrigger.aMathOps[i] = new JsonMathOp();
                condTrigger.aMathOps[i].strID = aMathOps[i].strID;
                condTrigger.aMathOps[i].strCond = CloneDeep(aMathOps[i].strCond, strReplace, strFind);
                condTrigger.aMathOps[i].nMathOp = aMathOps[i].nMathOp;
                condTrigger.aMathOps[i].fMathVal = aMathOps[i].fMathVal;
    ...
    [MonoModReplace] public bool Triggered(CondOwner objOwner, string strIAStatsName = null, bool logOutcome = true) {
        ...
        if (bAND) {
            if (strMathCond != null) {
                ...
                JsonMathOp[] refMathOps = aMathOps;
                foreach (JsonMathOp refMathOp in refMathOps) {
                    if (string.IsNullOrEmpty(refMathOp.strID)) continue;
                    if (refMathOp.strCond == null) {
                        int refMultOp = refMathOp.nMathOp;
                        double vFinal = refMathOp.fMathVal;
                        if (!MathTrigger(refMultOp, vMath, vFinal)) {
                            if (logReason) strFailReasonLast = $"Math Lacking: " +
                                $"{strMathCond} ({vMath}) is {MathToString(refMultOp)} {vFinal}";
                            return false;
                        }
                    } else if (objOwner.mapConds.TryGetValue(refMathOp.strCond, out refCond)) {
                        int refMultOp = refMathOp.nMathOp;
                        double vMult = refMathOp.fMathVal;
                        double vFinal = refCond.fCount * refMathOp.fMathVal;
                        if (!MathTrigger(refMultOp, vMath, vFinal)) {
                            if (logReason) strFailReasonLast = $"Math Lacking: " +
                                $"{strMathCond} ({vMath}) is {MathToString(refMultOp)} " +
                                $"{FormatTwoDecimals(vMult * 100.0)}% of {refMathOp} ({vFinal})";
                            return false;
                        }
                    }
                }
        ...
        if (strMathCond != null) {
            ...
            foreach (JsonMathOp refMathOp in refMathOps) {
                if (string.IsNullOrEmpty(refMathOp.strID)) continue;
                if (refMathOp.strCond == null) {
                    nMathOp = refMathOp.nMathOp;
                    vFinal = refMathOp.fMathVal;
                    if (MathTrigger(nMathOp, vMath, vFinal)) return true;
                } else if (objOwner.mapConds.TryGetValue(refMathOp.strCond, out refCond)) {
                    nMathOp = refMathOp.nMathOp;
                    vMult = refMathOp.fMathVal;
                    vFinal = refCond.fCount * vMult;
                    if (MathTrigger(nMathOp, vMath, vFinal)) return true;
                }
                if (logReason) {
                    if (refMathOp.strCond == null) {
                        if (bFirstAdded) strResult = $"{strResult}, " +
                        $"{strMathCond} is {MathToString(nMathOp)} {vFinal}";
                        else strResult = $"{strResult}" +
                        $"{strMathCond} is {MathToString(nMathOp)} {vFinal}";
                    } else {
                        if (bFirstAdded) strResult = $"{strResult}, " +
                        $"{strMathCond} is {MathToString(nMathOp)} " +
                        $"{FormatTwoDecimals(vMult * 100.0)}% of {refMathOp}";
                        else strResult = $"{strResult}" +
                        $"{strMathCond} is {MathToString(nMathOp)} " +
                        $"{FormatTwoDecimals(vMult * 100.0)}% of {refMathOp}";
                    }
    ...
    public string RulesInfo {
        [MonoModReplace] get {
            ...
            if (bAND) {
                ...
                if (strMathCond != null && aMathOps.Length > 0) {
                    ...
                    for (int i = 0; i < aMathOps.Length; i++) {
                        if (string.IsNullOrEmpty(aMathOps[i].strID)) continue;
                        if (i == 0) strBuilder.Append(" ");
                        else if (i == aMathOps.Length - 1) strBuilder.Append(", and ");
                        else if (i > 0) strBuilder.Append(", ");
                        if (aMathOps[i].strCond == null) {
                            strBuilder.Append(MathToString(aMathOps[i].nMathOp) + " ");
                            strBuilder.Append(aMathOps[i].fMathVal);
                        } else {
                            refCond = DataHandler.GetCond(aMathOps[i].strCond);
                            strBuilder.Append(MathToString(aMathOps[i].nMathOp) + " ");
                            strBuilder.Append(FormatTwoDecimals(aMathOps[i].fMathVal * 100.0) + "% of ");
                            strBuilder.Append(refCond.strNameFriendly);
                        }
                        if (i == aMathOps.Length - 1) strBuilder.Append(".");
                    }
                }
                ...
                if (strMathCond != null && aMathOps.Length > 0) {
                    ...
                    for (int i = 0; i < aMathOps.Length; i++) {
                        if (string.IsNullOrEmpty(aMathOps[i].strID)) continue;
                        if (i == 0) strBuilder.Append(" ");
                        else if (i == aMathOps.Length - 1) strBuilder.Append(", or ");
                        else if (i > 0) strBuilder.Append(", ");
                        if (aMathOps[i].strCond == null) {
                            strBuilder.Append(MathToString(aMathOps[i].nMathOp) + " ");
                            strBuilder.Append(aMathOps[i].fMathVal);
        ...
        get {
            ...
            if (bAND) {
                ...
                foreach (string strTrigger in refTriggersAnd) {
                    ...
                    strBuilder.Append("{" + refTrigger.RulesInfoDev + "}");
                }
            } else {
                for (int i = 0; i < aForbids.Length; i++) {
                    if (i == 0) {
